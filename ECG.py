...
class ECG:
    ...
    def plot_fft(self, individu="x", numECG="*", resultat="none"):
        ...
        plt.title("Individu : " + individu + " - FFT ECG : " + numECG + " - Résultat : " + resultat)
        plt.grid()
        plt.show()

    def plot_data_and_fft(self, individu="x", numECG="*", resultat="none"):
        plt.subplot(311)
        N = int(len(self.data))  # nombre d'element dans la liste
        T = (1 / self.sample)  # periode d'échantillonage du signal
        x = np.linspace(0.0, N * T, N)
    ...
    def plot_data_and_cpr(self):

        #listCPR = load_cpr_data()
        #if self.name.index('.')
